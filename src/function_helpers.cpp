...
void arg_list_helpers::asserters::assertion_failure(void *ctx, const char *msg) {
    throw js_exception(JS_ThrowTypeError(static_cast<JSContext *>(ctx), "%s", msg));
}

template <typename T> std::decay_t<T> pull_value_impl(JSContext *c, int argc, qjs::JSValue *argv, int i, bool strict) {
...
template <typename T> JSValue push_value_impl(JSContext *c, const T &v) {
    using H = value_helpers<std::decay_t<T>>;
    return H::from(c, v);
}
...
#define X(T)                                                                                                           \
    template <> T arg_list_helpers::getter<T>::get(void *ctx, int argc, qjs::JSValue *argv, int i, bool strict) {      \
        return pull_value_impl<T>(static_cast<JSContext *>(ctx), argc, argv, i, strict);                               \
    }                                                                                                                  \
    template <> qjs::JSValue arg_list_helpers::setter<T>::set(void *ctx, const T &v) {                                 \
